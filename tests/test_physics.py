...
def test_basic_insert_1_stats():
    ...
    tgc = eGC({'O': [['I', 0, 2]], 'U': [['I', 1, 2]]}, sv=False)
    igc = eGC({'O': [['I', 0, 2]], 'U': [['I', 1, 2]]}, sv=False)
    def f(): return md5(bytearray(pformat(_insert_gc(None, tgc, igc, 'A')[0]['graph']), encoding='ascii')).hexdigest()
...
def test_basic_insert_3_stats():
    ...
    tgc = mGC(igraph=gc_graph({'A': [['I', 0, 2], ['I', 1, 2]], 'O': [['A', 0, 2]]}), sv=False)
    igc = eGC({'O': [['I', 0, 2]], 'U': [['I', 1, 2]]}, sv=False)
    def f(): return md5(bytearray(pformat(_insert_gc(None, tgc, igc, 'B')[0]['graph']), encoding='ascii')).hexdigest()
    def func(x): return [f() for _ in range(x)]
    unlikely = 0
    for _ in range(3):
...
def test_basic_insert_6_stats():
    ...
    tgc = mGC(igraph=gc_graph(graph), sv=False)
    igc = eGC({'O': [['I', 0, 2]], 'U': [['I', 1, 2]]}, sv=False)
    def f(): return md5(bytearray(pformat(_insert_gc(None, tgc, igc, 'O')[0]['graph']), encoding='ascii')).hexdigest()
    def func(x): return [f() for _ in range(x)]
    unlikely = 0
    for _ in range(3):
        counts = Counter(func(STATS_N))

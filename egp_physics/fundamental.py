...
def clone(gc_to_clone: aGC, ref: Callable[[], int], copy_graph: bool = True) -> dGC:
    ...
    return {
        ...
        if gc_to_clone["gca_ref"] is not None
...
def proximity_select(gms: gene_pool, xputs: dict) -> aGC:
    ...
    if agc:
        if _LOG_DEBUG:
            _logger.debug(
                f"Proximity selection match_type {match_type} found a candidate."
            )
...
def steady_state_exception(gms: gene_pool, fgc: aGC) -> Work:
    ...
    if _LOG_DEBUG:
        _logger.debug(
            f"Steady state exception thrown for GC ref {ref_str(fgc.get('ref', 0))}."
        )
    fgc_gcg: gc_graph = fgc["gc_graph"]
    unconnected_eps: tuple[dst_end_point, ...] = tuple(
        fgc_gcg.i_graph.dst_unref_filter()
    )
    ...
    inputs = list(
        (ep[ep_idx.TYPE] for ep in filter(filter_func, fgc_gcg.graph.values()))
    )

    xputs = {"exclude_column": "signature", "exclusions": list()}
